...
class VisionInferenceService(Node):
    ...
    def __init__(self):
        ...
        # Resolve the YOLO model path. Accepts any of:
        #   - an absolute path  -> used as-is
        #   - a relative path   -> used as-is (relative to CWD)
        #   - a bare filename   -> resolved against this package's models/ folder,
        #                         so it works from any CWD and from src/, build/,
        #                         or install/ layouts.
        model_path_param = str(self.get_parameter('yolo_model').value).strip()
        candidate = Path(model_path_param)
        if not candidate.is_absolute() and not candidate.exists():
            # Bare filename: look in the vision_processing_package models/ folder.
            package_root = Path(__file__).resolve().parents[1]
            package_candidate = package_root / 'models' / candidate.name
