...
class GraspActionNode(Node):
    ...
    def _call_service(self, client, request, stage_name, timeout_sec=10.0, goal_handle=None):
        ...
        We interleave rclpy.spin_once() into the wait loop so the executor
        in this node can actually deliver the response — without it, the
        callback that completes the future never runs and we hang forever.
        """
        ...
        while rclpy.ok() and not future.done():
            ...
            if goal_handle is not None and goal_handle.is_cancel_requested:
                return None
            # spin_once processes any pending client callbacks (e.g. the one
            # that completes `future` when the server replies) so the
            # executor isn't starved while we busy-wait.
            try:
    ...
    def _hw_set_joint_state(self, angles, servo_ids, duration_vector, radians, goal_handle, label="SetJointState"):
        ...
        self.get_logger().info(
            f"[{label}] request servos={req.servo_ids} angles={req.angles} "
            f"durations={req.duration_vector}"
        )
        resp = self._call_service(
            self.set_joint_state_client, req, label,
            timeout_sec=15.0, goal_handle=goal_handle,
        )
        ok = bool(resp is not None and resp.success)
        if not ok:
            self.get_logger().error(
                f"[{label}] FAILED: resp={resp} (success={resp.success if resp is not None else 'None'})"
            )
        return ok
    ...
    def _hw_play_waypoints(self, waypoints, servo_ids, goal_handle):
        ...
        if not self.play_waypoints_client.wait_for_server(
            timeout_sec=GRASP_ACTION_WAIT_TIMEOUT_S
        ):
        ...
        _wait_for_future(send_future, GRASP_GOAL_ACCEPTANCE_TIMEOUT_S)
    ...
    def execute_callback(self, goal_handle):
        ...
        self.get_logger().info(
            f"Stage 0: mid-carry target angles={MidCarry.tolist()} servos={ARM_SERVO_IDS} "
            f"duration_ms={ARM_MID_CARRY_DURATION_MS}"
        )
        if not self._hw_set_joint_state(
            MidCarry, ARM_SERVO_IDS,
            [ARM_MID_CARRY_DURATION_MS] * len(ARM_SERVO_IDS), False, goal_handle,
            label="stage_0_mid_carry",
        ):
        ...
        if not self._hw_move_joint(ARM_WRIST_JOINT_INDEX, wrist_angle, goal_handle):
        ...
        if not self._hw_play_waypoints(best_motor_sets, ARM_TRAJ_SERVO_IDS, goal_handle):
            if goal_handle.is_cancel_requested:
                goal_handle.canceled()
            else:
        ...
        self.get_logger().info(
            f"Stage 7: returning to mid-carry angles={MidCarry.tolist()}"
        )
        if not self._hw_set_joint_state(
            MidCarry, ARM_SERVO_IDS,
            [ARM_MID_CARRY_DURATION_MS] * len(ARM_SERVO_IDS), False, goal_handle,
            label="stage_7_mid_carry",
        ):
        ...
        grasped = ee_count <= (GRIPPER_CLOSE_COUNT - GRIPPER_BLOCK_TOLERANCE)
        if grasped:
            self.get_logger().info(
                f"Object grasped — gripper_count={ee_count:.1f} "
                f"(<= {GRIPPER_CLOSE_COUNT - GRIPPER_BLOCK_TOLERANCE:.1f})"
            )
        else:
            self.get_logger().info(
