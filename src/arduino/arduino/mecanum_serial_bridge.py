...
class MecanumSerialBridge(Node):
    def __init__(self) -> None:
        ...
        self.cmd_rate_hz = self.get_parameter('cmd_rate_hz').value
        self.cmd_timeout_s = self.get_parameter('cmd_timeout_s').value
        self.stop_on_timeout = self.get_parameter('stop_on_timeout').value
    ...
    def _send_cmd_tick(self) -> None:
        ...
        if stale:
            ...
            return
        vx_cm = twist.linear.x * 100.0 # m/s (wskr) to cm/s (arduino)
        vy_cm = twist.linear.y * 100.0 # m/s (wskr) to cm/s (arduino)
